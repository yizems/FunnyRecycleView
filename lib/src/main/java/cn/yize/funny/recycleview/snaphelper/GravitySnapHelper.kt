package cn.yize.funny.recycleview.snaphelper
 ...
 * @property INVALID_DISTANCE Float
 * @property mVerticalHelper OrientationHelper?
 * @property mHorizontalHelper OrientationHelper?
 * @constructor
 */
class GravitySnapHelper(
    ...
    private fun distanceToCenter(
        ...
        val childCenter = getViewAnchor(targetView,layoutManager, helper)
        val containerCenter = getLayoutAnchor(layoutManager, helper)
    ...
    private fun findView(
        ...
        val layoutAnchor = getLayoutAnchor(layoutManager, helper)
        var absClosest = Int.MAX_VALUE
        for (i in 0 until childCount) {
            val child = layoutManager.getChildAt(i)
            val childAnchor = getViewAnchor(child,layoutManager, helper)
            val absDistance = abs(childAnchor - layoutAnchor)
    ...
    private fun getViewAnchor(
        view: View?,
        layoutManager: RecyclerView.LayoutManager,
        helper: OrientationHelper
    ): Int {
        view ?: return Int.MIN_VALUE
        return when (gravity) {
            Gravity.START -> helper.getDecoratedStart(view)
            Gravity.CENTER -> helper.getDecoratedStart(view) + helper.getDecoratedMeasurement(view) / 2
            Gravity.END -> helper.getDecoratedEnd(view)
        }
    }

    private fun getLayoutAnchor(
        layoutManager: RecyclerView.LayoutManager,
        helper: OrientationHelper
    ): Int {
        return when (gravity) {
